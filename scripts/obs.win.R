...
obs.win <- function(inputfile, auxfile = NULL, gastype = "CO2dry_ppm",
  ...
  time_filter$unixtime <- as.numeric(time_filter$POSIX.time)
  ...
  inputfile$unixtime <- as.numeric(inputfile$POSIX.time)

  # Add time_filter to inputfile and filter POSIXct
  # data.filter <- inputfile %>%
  #   right_join(time_filter, relationship = "many-to-many", by = "unixtime") %>%
  #   drop_na(matches(gastype))

  data.filter <- time_filter
  data.filter$CO2dry_ppm <- approx(inputfile$unixtime, inputfile$CO2dry_ppm, xout = data.filter$unixtime)$y
  data.filter$H2O_ppm <- approx(inputfile$unixtime, inputfile$H2O_ppm, xout = data.filter$unixtime)$y
  data.filter$CH4dry_ppb <- approx(inputfile$unixtime, inputfile$CH4dry_ppb, xout = data.filter$unixtime)$y
